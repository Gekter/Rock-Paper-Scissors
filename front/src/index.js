...
const randomName = new Set(shuffle(randomNames))
...
function getRandomName() {
  let notUsedNames = [...randomName].filter(x => !usingNames.has(x))
  usingNames.add(notUsedNames[0])
  return notUsedNames[0] ?? 'Ты кто вообще???'
}
...
socketObject.cbOnInit = (idUsers) => {
  let card;
  const messagesContainer = document.querySelector('.game-container tbody')
